...
public class FacebookController
{
	...
	public String getFromFb(@RequestParam(value = "code", required = false) String code)
	{
		...
		AccessToken accessToken = facebookClient.obtainUserAccessToken(facebookIdClient, facebookSecret, redirectUrl, code);
		facebookClient = new DefaultFacebookClient(accessToken.getAccessToken(), Version.VERSION_2_6);
		User user = facebookClient.fetchObject("me", User.class, Parameter.with("fields", "name,email,first_name,last_name,birthday,hometown,relationship_status,education,work,languages,picture"));
		
		Profile profile = facebookService.loginViaSocailNetwork(user);
		if (profile != null)
		{
		...
		profile = facebookService.signupViaSocailNetwork(user);
		if (profile != null)
		{
			SecurityUtil.authentificate(profile);
